...
```c
  if ( heap_list[idx] )
    free(heap_list[idx]->name);
```

版本是libc2.29，比起2.27变化其实不大

多了对tcache链表的double free的检查，因此UAF常见直接free两次或者间隔free两次的手法不行了

这里用的是[house_of_botcake.c](https://github.com/shellphish/how2heap/blob/master/glibc_2.26/house_of_botcake.c)的利用手法
...
![](<https://i.loli.net/2019/04/29/5cc62225e1ff2.png>)
