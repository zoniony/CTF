...
{
  mstate ar_ptr; //heap的Arena指针
  void *victim;
  //先检查__malloc_hook上是否值,有的话执行并退出
  //这也是当保护全开时利用的方法:修改hook
  /*不过第一次调用的时候__malloc_hook_指向的是malloc_hook_int

   28 malloc_hook_ini (size_t sz, const void *caller)
 ► 29 {
   30   __malloc_hook = NULL; 将_malloc_hook置为NULL,有点像延迟绑定的机制
   31   ptmalloc_init ();     初始ptmalloc,并会设立个标志__malloc_initialized为 1
   32   return __libc_malloc (sz); 再返回__libc_malloc(),此时malloc_hook为NULL
   33 }

   257 static void
   258 ptmalloc_init (void)
   259 {
 ► 260   if (__malloc_initialized >= 0) 下次进入再进入ptmalloc_init ()时则退出
   261     return;
  */
  ...
  //分配Arena,相当于知道了堆分配的地址

  arena_get (ar_ptr, bytes);
  //进入_int_malloc真正分配内存的函数
  victim = _int_malloc (ar_ptr, bytes);


  /* Retry with another arena only if we were able to find a usable arena
     before.  */
  //找到了分配的arena但是分配返回值为NULL失败,尝试寻找下一块可以用的arena
  if (!victim && ar_ptr != NULL)
    {
      LIBC_PROBE (memory_malloc_retry, 1, bytes);//不懂QQ
      ar_ptr = arena_get_retry (ar_ptr, bytes);//再去寻找合适的Arena
...
_int_malloc (mstate av, size_t bytes)
{
  ...
  /*checked_request2size()函数将需要分配的内存大小 bytes 转换为需要分配的 chunk 大小
    nb。
    Ptmalloc内部分配都是以 chunk 为单位，根据 chunk 的大小，决定如何获得满足条件的
    chunk。  
  */
   // 将用户所需大小bytes转化为chunk实际大小nb
   // nb = (bytes + size_T) algin (size_T * 2)

  checked_request2size (bytes, nb);

  /* There are no usable arenas.  Fall back to sysmalloc to get a chunk from
     mmap.  */
  /*分配arena失败,ptmalloc管理的堆空间不足,则调用sysmalloc向系统申请空间
    返回申请到的地址并退出
  */
  ...
  /*
    注释是神魔恋?即使av(分配的arena)没有初始化,这段代码也能成功运行,
    所以我们能尝试不检查,来节省时间
    这是因为在上一步av分配失败就退出了,所以fastbin就能安全检查少一点?
    所以Fastbin Attack容易的原因?
  */

  //四部曲第一步,nb符合fastbin范围,先检查fstbin中有没有合适的
  if ((unsigned long) (nb) <= (unsigned long) (get_max_fast ()))
    {
      idx = fastbin_index (nb);            //通过nb计算得到对应的fastbin下标
      mfastbinptr *fb = &fastbin (av, idx);//得到对应bins的地址
      //遍历fastbin单链表
      mchunkptr pp = *fb;
      do
        {
          victim = pp;
          if (victim == NULL) //fastbin的最后一个chunk的fd指向NULL
            break;            //结束循环
        }
        /*解释下catomic_compare_and_exchange_val_acq(fb, victim->fd, victim))
          if(fb == victim) fb = victim->fd,且返回值始终为fb的初始值
          结合返回fastbin头链表指针

          同时有个对double free的检查(pp != victim)
          链表的下一个地址不能为本身
       */
      ...
      if (victim != 0)
        {
          //chunksize (victim) 通过堆地址找到大小
          //fastb
          //不同则返回'malloc(): in_index (chunksize (victim) 通过大小计算出index
          //fastbin_index (chunksize (victim)) != idx 比较是否相同memory corruption (fast)'并退出

          /*
          这里也是fastbin Attack 需要绕过的检查,构造fake chunk时,size大小必须合适
          eg: *(&malloc_hook - 0x23) = 0x7f
          */
          ...
          //再检查,不过查资料说在DEBUG才用,暂时不管
          check_remalloced_chunk (av, victim, nb);
          //通过chunk2mem (victim)返回用户得到堆地址
          void *p = chunk2mem (victim);
          // 如果设置了 perturb_type , 则将获取到的chunk初始化为 perturb_type ^ 0xff
          // perturb_type 是什么东东QAQ
          alloc_perturb (p, bytes);
          return p;
        }
    }
  ...
  //走到这说明fastbin没找到,则继续搜索smallbin
  /*
      每个bin大小是固定的:Chunk_size = 2 * SIZE_SZ * index
      从bin[2]~bin[63]一共维护了62个双向链表的bins
      每个相邻的bins相差(2*SIZE_SZ)B
      32位:16B(2*8)~504bB(63*8)
      64位:32B(2*16)~1008B(63*16)
  */
  if (in_smallbin_range (nb))
    {
      ...
      */
      idx = smallbin_index (nb); //得到index
      bin = bin_at (av, idx);    //得到smallbin的的chunk指针

      /*
      因为是双向链表,得到头指针的bk指针指向最后一个chunk
      #define last(b) ((b)->bk)
      if(vimctim == bin)
      表明现在smallbin已经被初始化,而且这个bin为空
      if(vimctim != bin)此时有两种情况
      */
      if ((victim = last (bin)) != bin) 
        {
          //第一种:smallbin还没有初始化,则先初始化
          ...
          //smallbin中存在空闲的chunk,则取链表中最后一个chunk
            {
              //有个安全检查,检查趋要取出的chunk的上一块的fd指针是否指向它
              //防止伪造 :(
              bck = victim->bk;
	if (__glibc_unlikely (bck->fd != victim))
              ...
              //接下来是做取出的动作
              set_inuse_bit_at_offset (victim, nb); //设置inuse位,还是通过nb大小取寻找
              bin->bk = bck; //最后一块chunk指针指向倒数第二块
              bck->fd = bin; //倒数第二块的后一块指向头指针

              //如果不是main_arena,则设置mmap标志位
              if (av != &main_arena)
                victim->size |= NON_MAIN_ARENA;

              check_malloced_chunk (av, victim, nb); //DEBUG模式仔细检查
   ...
   */
  //注释的大概意思不要怪我直接合并fastbin很突兀,都是fastbin中防止碎片数据太多
  else//剩下的大小都不检查进行操作 = =!
    {
      idx = largebin_index (nb); //计算出largebin的index
      if (have_fastchunks (av))  //如果fastbin中有空闲的chunk
        malloc_consolidate (av); //调用malloc_consolidate将fastbin中的合并
                                 //清理fastbin,然后放到unsortbin
    }
   ...
   */
    //前面fastbin和smallbin中都没有大小精确的可以分配,接下来考虑
    //1.处理unsortbin
    //2.处理largebin
  for (;; )
    {
      int iters = 0; //循环次数
      //有unsortbin的话
      //1.反向遍历unsortbin,直到遍历第一个
      //2.安全检查
      //3.还属于remainder_chunk大小足够则分割
      //4.分配到bins
      while ((victim = unsorted_chunks (av)->bk) != unsorted_chunks (av))
        {
          bck = victim->bk;//victim的上一个
          //先进行安全检查
          //1.大小不能小于或等于最小的chunk大小2 * SIZE_SZ
          //2.不能大于分配的总分配量av->system_mem
          if (__builtin_expect (victim->size <= 2 * SIZE_SZ, 0)
              || __builtin_expect (victim->size > av->system_mem, 0))
            malloc_printerr (check_action, "malloc(): memory corruption",
                             chunk2mem (victim), av);
          size = chunksize (victim);//得到victim对应的chunk大小。
           ...
           */
          //注释的意思是属于small bin大小的chunk还可以在unsortbin中捡漏,化利于 cpu 的高速缓存命中?
          if (in_smallbin_range (nb) &&       //大小在smallbin范围内
              bck == unsorted_chunks (av) &&  //仅有一块unsortbin chunk
              victim == av->last_remainder && //并且还是last_remainder chunk
              (unsigned long) (size) > (unsigned long) (nb + MINSIZE)) //而且大小足够分配 
            {
              ...
              //剩下remainder为large chunk的话,将fd_nextsize和bk_nextsize置为NULL,因为remainder只有一块chunk
              if (!in_smallbin_range (remainder_size))
                {
                  remainder->fd_nextsize = NULL;
                  remainder->bk_nextsize = NULL;
                }
              //这两部不算太明白XD,以后有机会碰见再调调看
              //设置victim的头部
          ...
          //将unsorted最后一块chunk取下来 
          unsorted_chunks (av)->bk = bck;
          bck->fd = unsorted_chunks (av);

          /* Take now instead of binning if exact fit */
          //取出的unsort_chunk和所需的chunk大小刚好
          if (size == nb)
            {
              //那就使用这块,先标记inuse
              set_inuse_bit_at_offset (victim, size);
              //不是main_arena再设置arena位
              if (av != &main_arena)
                victim->size |= NON_MAIN_ARENA;
              //三部曲
              check_malloced_chunk (av, victim, nb);
          ...
          /* place chunk in bin */
          //把unsortbin放入small bin或者largebin中
          //放入smallbins
          if (in_smallbin_range (size))
            {
              //计算出index
...
#define MAX_ITERS       10000
       ...
       */
      //走到这说明,bins(fastbin,smallbin,largebin)中都没有找到合适的
      //则尝试在下一个相邻的bins中找找有没有合适的chunk
      /*
          binmap[]是用来加快查找bins中是否存在空闲chunk的数组
          用block来管理,分为4个int,也就是32byte,每个byte对应一个bin
      */
      ++idx;                        //下一个bins
      bin = bin_at (av, idx);       //得到相应指针
      block = idx2block (idx);      //得到对应block
      map = av->binmap[block];      //通过block看map是否使用
      bit = idx2bit (idx);          //
    ...
    use_top:
       ...
       */
      //如果所有的bin都找不到合适的chunk,那只有从topchunk申请
      victim = av->top;         //先找到topchunk的指针
      size = chunksize (victim);//接着是大小

      //如果topchunk大小满足需要分配的大小
      if ((unsigned long) (size) >= (unsigned long) (nb + MINSIZE))
        {
          remainder_size = size - nb;
          remainder = chunk_at_offset (victim, nb);
      ...
      //合并atomic ops(原子操作???)free后的fastbin???
      //不是前面在进入大循环前已经malloc_consolidate合并了fastbin
      else if (have_fastchunks (av))
        {
          malloc_consolidate (av);
...
_int_free (mstate av, mchunkptr p, int have_lock)
{
  ...
  size = chunksize (p);//先得到需要释放chunk的大小

  /* Little security check which won't hurt performance: the
     allocator never wrapps around at the end of the address space.
     Therefore we can exclude some size values which might appear
     here by accident or by "design" from some intruder.  */
  //....在不影响性能的情况下的一些轻量级的安全检查,在安全和性能折中更偏向于性能= =#
  //1.指针地址 > -size ???
  //2.指针地址必须对齐
  ...
  /* We know that each chunk is at least MINSIZE bytes in size or a
    ...
    //在进行大小的检查
    //1.size必须大于最小的MINSIZE
    //2.分配的时候size是对齐的,free的时候也是啦
  ...
  */
  //处理size属于fastbin的范围,则就插入到第一个fastbin头部
  //注意操作没有改变inuse位
  if ((unsigned long)(size) <= (unsigned long)(get_max_fast ())
	...
	bordering top into fastbins
      */
      //TRIM_FASTBINS默认为0是不开启的
      && (chunk_at_offset(p, size) != av->top)
#endif
      ) {
    //相邻的下一个chunk大小小于MIX
    //相邻的下一个chunk大小大于系统分配的大小,一般是132K
   ...
   //分配区加锁???
    ...
    //接下来的操作就是将chunk插入fastbin
    //这里使用了Local_free算法进行插入链表
	...
	/* Check that the top of the bin is not the record we are going to add
	   (i.e., double free).  */
  //防止double free
  //old代表链表头chunk的地址,p代表需要插入chunk的地址
  //double free所以只要中间隔一个就很容易绕过检查
    ...
    //还确保加入后的chunk和加入之前的相同,通过size确定的indexhttps://ctf-wiki.github.io/ctf-wiki/pwn/heap/heap_implementation_details/#mmap-chunk
  ...
  */
  //合并非 mmap 的空闲 chunk,也就是喜闻乐见的unlink
  //说明只有不是fastbin的情况下才能unlink
  ...
  //1.当前chunk非mmap
    ...
    //free的chunk的下一个chunk不能超过arena边界
    //1.当前的chunk是sbrk分配的
    //2.下一块chunk超过了top_chunk
    ...
    //肯定还要检查inuse位,保证free这块处于使用状态
    //想到幸亏fastbin不改变inuse,没有这项检查,不然double_free就没法用了
    ...
    //将指针的mem设置为perturb_byte 
    free_perturb (chunk2mem(p), size - 2 * SIZE_SZ);

    /* consolidate backward */
    //前一块(低地址)chunk是free的向前合并  
    if (!prev_inuse(p)) {
      //前一块chunk的大小
      //这可以搞事?
      ...
      if (__glibc_unlikely (fwd->bk != bck))
	{
      ...
      p->fd = fwd;
      p->bk = bck;
      //吐过属于Large Bin的话fd_nextsize和bk_nextsize设置位NULL
      if (!in_smallbin_range(size))
	{
	  p->fd_nextsize = NULL;
	  p->bk_nextsize = NULL;
    ...
    */
    //否则合并到topchunk,修改top_chunk指针和大小
    else {
      size += nextsize;
      set_head(p, size | PREV_INUSE);
      av->top = p;
    ...
    /*
      ...
      is reached.
    */
    //如果free的大小或者合并后的大小大于FASTBIN_CONSOLIDATION_THRESHOLD(64K)
    //1.先收回fastbin中空闲的chunk
    //2.再收回查过阈值的空间
    if ((unsigned long)(size) >= FASTBIN_CONSOLIDATION_THRESHOLD) {
      if (have_fastchunks(av))
    //则合并fastbin中空闲的chunk放到unsortbin中,
    //free和malloc都可能malloc_consolidate
	malloc_consolidate(av);

      //1.处于主分配区
      //2.top_chunk大于收缩的heap阈值
      if (av == &main_arena) {
